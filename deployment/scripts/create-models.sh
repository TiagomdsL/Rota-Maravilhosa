...
OPTIONS(model_type='linear_reg', input_label_cols=['Severity']) AS
SELECT 
...
# Modelo de ocorrência 2 (corrigido - usar linear_reg porque só tem 1 label)
...
EOSQL
echo "✅ Modelo de ocorrência 2 criado!"

# Modelo de severidade 2 (corrigido - nome da coluna Visibility_mi_)
...
SELECT
  ...
  Severity
FROM `proj1cc-493515.accidents.accidents`
WHERE Visibility_mi_ IS NOT NULL
  AND Precipitation_in_ IS NOT NULL
  AND Weather_Condition IS NOT NULL
  AND Severity IS NOT NULL
EOSQL
echo "✅ Modelo de severidade 2 criado!"

# Modelo de risco 2 (corrigido - usar linear_reg)
...
echo ""
echo "✅ Todos os modelos criados com sucesso!"
